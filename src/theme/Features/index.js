...
const data = [
  {
    ...
    description: (
      <>
        scVIVA-Tools packages scVIVA, ResolVI, DestVI, DiagVI, Harreman, gimVI, Stereoscope, and Tangram —
        deconvolution, denoising, niche-aware differential expression, gene imputation, and cell-cell
        communication — behind a single, consistent API.
      </>
    ),
  },
  {
    ...
    description: (
      <>
        Every model follows the same setup/train/extract pattern, so switching from deconvolution with
        DestVI to niche-aware modeling with scVIVA means changing the class name, not relearning an API.
      </>
    ),
  },
  {
  ...
  },
  {
    ...
    description: (
      <>
        Neighbor-graph-heavy steps (e.g. ResolVI, scVIVA) can use GPU acceleration via the optional{" "}
        <code>rapids</code> extra (cuML/cuPy/cuGraph), so analyses scale with dataset size.
      </>
    ),
  },
];
